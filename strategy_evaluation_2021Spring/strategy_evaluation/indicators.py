...
#Bollinger Bands
def Bollinger(sd=dt.datetime(2008,1,1),ed=dt.datetime(2009,12,31),syms=['JPM'],gen_plot=False):
    # get data
...
#SMA
def SMA(sd=dt.datetime(2008,1,1),ed=dt.datetime(2009,12,31),syms=['JPM'],gen_plot=False):
    dates = pd.date_range(sd, ed)
    symbol = syms
    price = get_data([symbol], dates)
    price.fillna(method="ffill", inplace=True)
...
# momentum
def momentum(sd=dt.datetime(2008,1,1),ed=dt.datetime(2009,12,31),syms=['JPM'],gen_plot=False):
    dates = pd.date_range(sd, ed)
    symbol = syms
    price = get_data([symbol], dates)
    price.fillna(method="ffill", inplace=True)
    ...
    momentum=price/price.shift(periods = 20) -1
...
# volatility
def STD(sd=dt.datetime(2008,1,1),ed=dt.datetime(2009,12,31),syms=['JPM'],gen_plot=True):
    dates = pd.date_range(sd, ed)
    symbol = syms
    price = get_data([symbol], dates)
    price.fillna(method="ffill", inplace=True)
...
# EMA
def EMA(sd=dt.datetime(2008, 1, 1), ed=dt.datetime(2009, 12, 31), syms=['JPM'], gen_plot=True):
    dates = pd.date_range(sd, ed)
    symbol = syms
    price = get_data([symbol], dates)
    price.fillna(method="ffill", inplace=True)
