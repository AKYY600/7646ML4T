...
def compute_portvals(  		  	   		   	 			  		 			     			  	  		 	  	 		 			  		  			
    ...
    # TODO: Your code here  		  	   		   	 			  		 			     			  	  		 	  	 		 			  		  			
    df = pd.read_csv(orders_file,index_col="Date",na_values=["nan"],parse_dates=True)
    print (df.index)
    ...
    symbol_price = symbol_price[symbols]
    ...
    for index, row in df.iterrows():
        if index not in pd.date_range(start_date,end_date):
            continue
        if row['Order'] == 'BUY':
            trades.ix[index,row['Symbol']] += row['Shares']
            money = commission + symbol_price.ix[index,row['Symbol']] * (1+impact) * row['Shares']
            trades.ix[index,'cash'] -= money
        else:
            trades.ix[index, row['Symbol']] -= row['Shares']
            money = symbol_price.ix[index,row['Symbol']] * (1-impact)*row['Shares'] - commission
            trades.ix[index,'cash'] += money
